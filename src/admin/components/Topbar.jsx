    ...
    <header className="flex h-16 flex-shrink-0 items-center gap-3 border-b border-border bg-surface px-4 md:px-6">
      <button
        ...
        className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg text-muted
                   hover:bg-surface-raised hover:text-primary lg:hidden focus-visible:outline-none
      ...
      <div className="relative flex w-full max-w-[160px] items-center sm:max-w-xs md:max-w-sm">
        ...
        <input
          ...
          className="w-full rounded-xl border border-border bg-surface-raised py-2 pl-9 pr-3 text-sm text-primary
        ...
        {showSearchResults && matches.length > 0 && (
          <div className="absolute top-full left-0 z-30 mt-1 w-full overflow-hidden rounded-xl border border-border
                          bg-surface-raised shadow-lg">
            {matches.map((d) => (
              <button
                key={d.id}
                onClick={() => {
                  navigate(`/admin/devices/${d.id}`);
                  setSearch('');
                }}
